...
function App() {
  ...
  const submit = async () => {
    //type.replace(/-/g, "/");
    await axios
      .get(`http://localhost:3011/getimage?type=${type}`)
      .then((res) => {
        setImage(res.data);
      });
  };
  ...
  return (
    <div className="App" style={{}}>
      ...
      >
        ...
        >
          Find random picture of dogs below:
        </p>
        ...
        <select
          ...
          onChange={(event) => setType(event.target.value.replace(/-/g, "/"))}
          name="select one"
        >
        ...
        {image.length > 0 ? (
          <img style={{ borderRadius: "1em", maxHeight: "50%" }} src={image} />
        ) : (
          <p />
        )}
      </div>
